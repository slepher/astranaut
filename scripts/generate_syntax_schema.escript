...
main(Args) ->
    try
        ...
        Class:Reason:Stacktrace ->
            io:format(
                standard_error,
                "syntax schema generation failed: ~p:~tp~n~tp~n",
                [Class, Reason, Stacktrace]
            ),
            halt(2)
    end.
...
generate_forms(Schema) ->
    ...
    [
        ...
        function_form(
            attribute_layout,
            attribute_layout_clauses(
                maps:get(attribute_layouts, Schema),
                Schema
            )
        )
    ].
...
validate_generated_forms(Forms) ->
    case compile:forms(Forms, [return_errors, return_warnings]) of
        ...
        {error, Errors, Warnings} -> erlang:error({invalid_generated_forms, Errors, Warnings})
    end.
...
role_entries(Nodes, Excluded) ->
    ...
    lists:sort([
        {maps:get(type, E), lists:usort(maps:get(roles, E))}
     || E <- Entries
    ]).
        ...
        clause(
            ...
            [
                case_expr(
                    ...
                    [
                        clause(
                            [tuple_ast([Since, Until])],
                            [],
                            [
                                andalso_expr(
                                    op_expr('>=', OtpVsn, Since),
...
node_bounds_clauses(Nodes, Schema) ->
    ...
    Bounds = [
        {maps:get(type, Node), maps:get(since, Node, SchemaMin), maps:get(until, Node, SchemaMax)}
     || Node <- Nodes
    ],
    [
...
structural_slot_entries(Nodes, Schema, StructuralTypes) ->
    ...
    case
        [
            Entry
         || {_ParentType, _Slot, ChildType} = Entry <- Entries,
            not lists:member(ChildType, StructuralTypes)
        ]
    of
        [] -> Entries;
...
shape_pattern({value, native_record_name}, Next) ->
    ...
    Name = variable("Record", Next + 1),
    {
        ...
        [local_call(is_atom, [Module]), local_call(is_atom, [Name])],
        Next + 2
    };
shape_pattern({value, anonymous_record_name}, Next) ->
    {literal([]), [], Next};
shape_pattern({value, _Name}, Next) ->
    {variable('_'), [], Next};
...
shape_pattern({optional, Shape}, Next) ->
    shape_pattern(Shape, Next);
        ...
        [
            clause(
                [
                    variable('Type'),
                    variable('Subtrees'),
                    variable('_ParentRole'),
                    variable('_OtpVsn')
                ],
                [],
                [
                    error_tuple(
                        invalid_syntax_layout,
...
attribute_child_layout_clause() ->
    ...
    OtpVsn = variable('OtpVsn'),
    Try =
        {'try', 0, [remote_call(erl_syntax, atom_value, [NameTree])],
            [
                clause(
                    [variable('Name')],
                    [],
            ...
            [
                clause(
                    [tuple_ast([variable('_'), variable('_'), variable('_')])],
                    [],
                    [
                        error_tuple(
...
layout_clause(Type, Layout, Variant, SchemaMin, SchemaMax) ->
    ...
    ParentRole =
        case maps:get(when_role, Variant, any) of
    ...
    GroupMode =
        case maps:get(groups, Layout) of
            any -> groups;
            _ -> nodes
        end,
...
group_pattern(Index, many) ->
    variable("Group", Index);
group_pattern(Index, deep_many) ->
    variable("Group", Index).

descriptors(Children, Groups, ParentRole, GroupMode) ->
    [
        tuple_ast([
            literal(maps:get(slot, Child)),
            role_ast(maps:get(role, Child), ParentRole),
            Group,
            literal(GroupMode)
        ])
...
version_guards(Since, Until, SchemaMin, SchemaMax) ->
    Lower =
        case Since =:= SchemaMin of
            true -> [];
            false -> [op_expr('>=', variable('OtpVsn'), literal(Since))]
        end,
    Upper =
        case Until =:= SchemaMax of
            true -> [];
...
attribute_clauses(Name, Layout, SchemaMin, SchemaMax) ->
    ...
    NameDescriptor = tuple_ast([
        literal(name),
        literal(name),
        list_ast([variable('NameTree')]),
        literal(nodes)
    ]),
    [
        clause(
            ...
            VersionGuards,
            [ok_tuple([NameDescriptor | Descriptors])]
        )
    ] ++
        attribute_fallback_clauses(
            Name,
            BodyPattern,
            Since,
            Until,
...
attribute_body_pattern([Child | Children], Index, PatternAcc, GroupAcc) ->
    ...
    case maps:get(cardinality, Child) of
        one ->
            attribute_body_pattern(
                Children,
                Index + 1,
                [Group | PatternAcc],
                [list_ast([Group]) | GroupAcc]
            );
        many when Children =:= [] ->
...
resolve_layouts(#{layouts := Layouts}, _Schema, _Nodes) ->
    Layouts;
...
resolve_formats(Node, _Nodes) ->
    maps:get(formats, Node, []).
