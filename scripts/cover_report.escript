...
main(_Args) ->
    ...
    Reports = lists:filtermap(fun(Mod) ->
    ...
    io:format("Coverage report -> ~s~n  ~p modules, ~.1f%~n",
...
build_report(Mod, Cov, NotCov, Result) ->
    ...
    #{module => atom_to_list(Mod), coverage => Pct,
      covered => Cov, not_covered => NotCov, total => Total, missed => Missed}.

count_lines([]) -> {0, 0};
count_lines([{{_M, L}, {C, N}} | T]) when L > 0 ->
    {CT, NT} = count_lines(T), {C + CT, N + NT};
count_lines([_ | T]) -> count_lines(T).

src_lines(Mod) ->
    case find_src(Mod) of
        undefined -> fun(_Line) -> null end;
        Path ->
            {ok, Bin} = file:read_file(Path),
            Lines = binary:split(Bin, <<"\n">>, [global]),
            fun(Line) when Line =< length(Lines) ->
...
find_src_1([]) -> undefined;
find_src_1([P|Ps]) -> case filelib:is_file(P) of true -> P; false -> find_src_1(Ps) end.

total_pct([]) -> 0.0;
total_pct(Reports) ->
    {TC, TN} = lists:foldl(fun(#{covered := C, not_covered := N}, {CA, NA}) ->
        {CA+C, NA+N} end, {0,0}, Reports),
    case TC+TN of 0 -> 100.0; T -> round(TC * 10000 / T) / 100 end.
...
mod_to_json(#{module := Mod, coverage := Pct, covered := Cov,
              not_covered := Not, total := Total, missed := Missed}) ->
    Ms = [missed_to_json(L, C) || {L, C} <- Missed],
    <<"    { \"module\": \"", (list_to_binary(Mod))/binary, "\",\n",
      "      \"coverage\": ", (float_bin(Pct))/binary, ",\n",
...
missed_to_json(Line, Code) ->
    <<"        { \"line\": ", (integer_to_binary(Line))/binary,
      ", \"code\": \"", (json_escape(unicode:characters_to_binary(Code, utf8)))/binary, "\" }">>.
