...
init_per_suite(Config0) ->
    ...
    TestModules =
        [
            ...
            macro_uniform_b,
            macro_pass_boot,
            macro_pass_generated,
            macro_pass_depth,
            macro_pass_test,
            macro_pass_attribute_buffer_test,
            macro_pass_no_backscan_test,
            macro_pass_local_chain_test,
            ...
            macro_pass_external_helper,
            macro_pass_inject_attrs,
...
all() ->
    [
        ...
        test_local_macro_compile_does_not_wait_for_source_module_lock
    ].
...
test_macro_pass_closure_roots_missing_error(Config) ->
    assert_macro_pass_error(
        macro_pass_closure_roots_missing_error_test,
        Config,
        fun
...
test_macro_pass_local_body_environment_mutation_error(Config) ->
    assert_macro_pass_error(
        macro_pass_local_body_env_error_test,
        Config,
        fun
            (
        ...
        [{1, astranaut_macro, {missing_macro_formatter, macro_pass_local_body_env_error_test}}]
    ),
    ok.

test_macro_pass_locked_spec_mutation_error(Config) ->
    assert_macro_pass_error(
        macro_pass_locked_spec_error_test,
        Config,
        fun
            ({illegal_locked_form_mutation, {attribute, _, spec, {{helper, 1}, _}}}) ->
                true;
            (_) ->
                false
        end,
        [{1, astranaut_macro, {missing_macro_formatter, macro_pass_locked_spec_error_test}}]
    ),
    ok.
...
test_macro_pass_scoped_attribute_state(_Config) ->
    ...
    ?assertMatch(
        {tuple, _, [
            {atom, _, external_generated_helper},
            {atom, _, ok}
        ]},
...
test_macro_pass_generated_macro_options(Config) ->
    assert_macro_pass_error(
        macro_pass_generated_options_error_test,
        Config,
        fun
            ({max_macro_expansion_depth_exceeded, {macro_pass_depth, chain_a}, []}) ->
                true;
            (_) ->
                false
        end,
        [
            {3, astranaut_macro, {missing_macro_formatter, macro_pass_boot}},
            {5, astranaut_macro, {missing_macro_formatter, macro_pass_depth}}
        ]
    ),
    ok.
...
test_macro_pass_attribute_buffer_cross_depth(Config) ->
    assert_macro_pass_error(
        macro_pass_attribute_buffer_cross_depth_error_test,
        Config,
        fun
            ({max_macro_expansion_depth_exceeded, {macro_pass_depth, buffer_chain}, [_]}) ->
                true;
            (_) ->
                false
        end,
        [{2, astranaut_macro, {missing_macro_formatter, macro_pass_depth}}]
    ),
    ok.

test_macro_pass_attribute_buffer_self_depth(Config) ->
    assert_macro_pass_error(
        macro_pass_attribute_buffer_self_depth_error_test,
        Config,
        fun
            ({max_macro_expansion_depth_exceeded, {macro_pass_depth, buffer_self}, [_]}) ->
                true;
            (_) ->
                false
        end,
...
test_macro_pass_attribute_buffer_total_depth(Config) ->
    assert_macro_pass_error(
        macro_pass_attribute_buffer_total_depth_error_test,
        Config,
        fun
            ({max_macro_expansion_depth_exceeded, {macro_pass_depth, buffer_siblings}, [_]}) ->
                true;
            (_) ->
                false
        end,
...
test_macro_pass_export_helper_unlocked(Config) ->
    ...
    ?assert(
        lists:any(
            fun
                ({_Line, erl_lint, {redefine_spec, {helper, 1}}}) ->
                    true;
                (_) ->
                    false
            end,
    ...
    ?assertNot(
        lists:any(
            fun
                ({_Line, astranaut_macro, {illegal_local_macro_definition_mutation, _}}) ->
                    true;
                (_) ->
                    false
            end,
    ...
    ?assertEqual(
        [{4, astranaut_macro, {missing_macro_formatter, macro_pass_export_helper_unlocked_test}}],
        Warnings
    ),
    ok.
...
test_macro_pass_local_environment_mutation_errors(Config) ->
    ...
    ?assertEqual(
        [{1, astranaut_macro, {missing_macro_formatter, macro_pass_local_import_error_test}}],
        Warnings1
    ),
    {[{_File, [{3, astranaut_macro, {undefined_macro, generated_local, 0}}]}], [
        {_WarningFile2, [
            {1, astranaut_macro, {missing_macro_formatter, macro_pass_local_macro_error_test}}
        ]}
    ]} =
        astranaut_test_lib:realize_with_baseline(
...
test_macro_pass_locked_snapshot_mutation_error(Config) ->
    assert_macro_pass_error(
        macro_pass_locked_helper_error_test,
        Config,
        fun
            ({illegal_locked_form_mutation, {function, _, helper, 1, _}}) ->
                true;
            (_) ->
                false
        end,
        [{1, astranaut_macro, {missing_macro_formatter, macro_pass_locked_helper_error_test}}]
    ),
    ok.

test_local_macro_compile_does_not_wait_for_source_module_lock(_Config) ->
    Module = macro_local_unique_compile_test,
    Parent = self(),
    Holder =
        spawn(
            fun() ->
                astranaut_lib:with_module_lock(
                    Module,
                    fun() ->
                        Parent ! source_module_lock_held,
                        receive
                            release -> ok
                        end
                    end
                ...
                ?assertEqual(
                    [
                        {"macro_local_unique_compile_test.erl", [
                            {2, astranaut_lib,
                                {astranaut_macro,
                                    {missing_macro_formatter, macro_local_unique_compile_test}}}
                        ]}
                    ],
                    Warnings
                );
...
assert_macro_pass_error(Module, Config, MatchError, ExpectedWarnings) ->
    ...
    {[{_File, Errors}], WarningFiles} =
        astranaut_test_lib:realize_with_baseline(Baseline, ErrorStruct),
    Warnings = lists:append([
        FileWarnings
     || {_WarningFile, FileWarnings} <-
            WarningFiles
    ]),
    ?assert(
        lists:any(
            fun
                ({_Line, astranaut_macro, Error}) ->
                    MatchError(Error);
                (_) ->
                    false
            end,
...
local_macro_lifecycle_forms(Module, Value) ->
    ...
    [
        ...
        {function, 3, macro_value, 0, [{clause, 3, [], [], [erl_parse:abstract({atom, 3, Value})]}]},
        {function, 4, value, 0, [{clause, 4, [], [], [{call, 4, {atom, 4, macro_value}, []}]}]}
    ].
