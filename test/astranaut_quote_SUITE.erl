...
all() ->
    [
        ...
        test_literal_tuple,
        test_pattern_match,
        test_pattern_function_1,
        test_pattern_function_2,
        test_pattern_function_3,
        test_pattern_case_1,
        test_pattern_case_2,
        test_pattern_case_3,
        test_function_expression,
        test_function_expression_error,
        test_named_function_expression_1,
        test_named_function_expression_2,
        test_pos_1,
        test_pos_2,
        test_unquote,
        test_unquote_map,
        test_unquote_map_match,
        test_unquote_map_match_list,
        test_unquote_record,
        test_unquote_record_match,
        test_unquote_record_match_list,
        test_binding,
        test_atom_binding,
        test_dynamic_binding,
        test_dynamic_binding_pattern,
        test_unquote_splicing_1,
        test_unquote_splicing_2,
        test_unquote_splicing_map,
        test_type,
        test_type_atom,
        test_type_map,
        test_type_tuple,
        ...
        test_record,
        test_native_records,
        test_native_record_attributes,
        test_spec,
        test_callback,
        test_opaque,
        test_empty_quote_code,
        test_empty_quote_type_code,
        test_quote_code_options_must_be_last,
        test_invalid_binding_values,
        test_quote_public_helpers,
        test_quote_code_options_last,
...
test_function_expression_error(_Config) ->
    ?assertException(
        error, {unexpected_type_of_var, _, atom_value, 233}, quote_example:function_expression(233)
    ),
    ok.
...
test_named_function_expression_1(_Config) ->
    ...
    Ast = merl:quote(
        0,
        "fun H(0) -> 0; H(N@astranaut_quote@quote_example) -> H(N@astranaut_quote@quote_example - 1) + N@astranaut_quote@quote_example end"
    ),
    ?assertEqual(Ast, Expression),
    ok.
...
test_named_function_expression_2(_Config) ->
    ...
    Ast = merl:quote(
        0,
        "fun Name@astranaut_quote@quote_example(0) -> 0; Name@astranaut_quote@quote_example(N@astranaut_quote@quote_example) -> Name@astranaut_quote@quote_example(N@astranaut_quote@quote_example - 1) + N@astranaut_quote@quote_example end"
    ),
    ?assertEqual(Ast, Expression),
    ok.
...
test_pos_2(_Config) ->
    ...
    ?assertEqual(
        {tuple, 12, [{atom, 12, ok}, {tuple, 13, [{atom, 13, hello}, {atom, {10, 11}, world}]}]},
        HelloWorld
    ),
    ok.
...
test_native_records(_Config) ->
    ...
    ?assertMatch(
        {record, _, {mod, rec}, [{record_field, _, {atom, _, x}, {integer, _, 1}}]},
        QualifiedCreate
    ),
    ?assertMatch(
        {record, _, {var, _, _}, {mod, rec}, [{record_field, _, {atom, _, x}, {integer, _, 2}}]},
        QualifiedUpdate
    ),
    ?assertMatch(
        {record, _, [], [{record_field, _, {atom, _, x}, {var, _, _}}]},
        AnonymousPattern
    ),
    ?assertMatch(
        {record, _, {var, _, _}, [], [{record_field, _, {atom, _, x}, {integer, _, 3}}]},
        AnonymousUpdate
    ),
    ?assertMatch(
        {record_field, _, {var, _, _}, {mod, rec}, {atom, _, x}},
        QualifiedAccess
    ),
    ?assertMatch(
        {record_field, _, {var, _, _}, [], {atom, _, x}},
        AnonymousAccess
    ),
    ok.
...
test_native_record_attributes(_Config) ->
    ...
    ?assertMatch(
        {attribute, _, native_record, {rec, [{record_field, _, {atom, _, x}}]}},
        NativeRecord
    ),
...
test_quote_code_options_last(_Config) ->
    ...
    {function, _, static, 0, [{clause, _, [], [], [StaticExpression]}]} =
    ...
    {function, _, dynamic, 1, [{clause, _, [{var, _, 'Pos'}], [], [DynamicExpression]}]} =
...
test_quoted_invalid_unquote_splicing_warning(_Config) ->
    ...
    ?assertMatch(
        {10, astranaut_quote, {invalid_unquote_splicing, {var, 10, 'A'}}},
        Warning
    ),
...
test_quoted_invalid_unquote_splicing_binding_warning(_Config) ->
    ...
    {warning, _Return, [Warning]} = astranaut_quote:quoted(Node),
    ?assertEqual(
        {20, astranaut_quote, {invalid_unquote_splicing_binding, 'As'}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_quoted_type_binding_warning(_Config) ->
    Node =
        {attribute, 30, type,
            {foo, {remote_type, 31, [{atom, 32, '_V@Module'}, {atom, 33, '_A@Type'}, []]}, []}},
    {warning, _Return, [Warning]} = astranaut_quote:quoted(Node),
    ?assertEqual(
        {32, astranaut_quote, {only_bindings_supported, ["A", ""], 'Module', '_V@Module'}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_quoted_pattern_splicing_tail_warning(_Config) ->
    ...
    ?assertEqual(
        {40, astranaut_quote, {unquote_splicing_pattern_non_empty_tail, [{atom, 40, tail}]}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_parse_transform_tuple_pos_warning(_Config) ->
    Forms =
        [
            ...
            {function, 60, run, 0, [
                {clause, 60, [], [], [{call, 60, {atom, 60, quote}, [{foo, bar}]}]}
            ]},
            {eof, 61}
        ],
    {warning, _QuotedForms, [{_File, [Warning]}]} =
        astranaut_quote:parse_transform(Forms, []),
    ?assertEqual(
        {60, astranaut_lib, {astranaut_quote, {could_not_get_tuple_pos_value, {foo, bar}}}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_parse_transform_literal_name_binding_warning(_Config) ->
    ...
    {warning, _QuotedForms, [{_File, [Warning]}]} =
        astranaut_quote:parse_transform(Forms, []),
    ?assertEqual(
        {4, astranaut_lib, {astranaut_quote, {only_bindings_supported, ["A"], 'Name', '_V@Name'}}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_quoted_tuple_pos_warning(_Config) ->
    ...
    ?assertEqual(
        {bar, astranaut_quote, {could_not_get_tuple_pos_value, {foo, bar}}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
...
test_quoted_literal_name_binding_warning(_Config) ->
    ...
    {warning, _Return, [Warning]} = astranaut_quote:quoted(Node),
    ?assertEqual(
        {70, astranaut_quote, {only_bindings_supported, ["A"], 'Name', '_V@Name'}},
        Warning
    ),
    astranaut_test_lib:assert_formatted_messages([Warning]),
    ok.
    ...
    ?assertMatch(
        {error,
            [
                {_, [
                    {_, astranaut_lib,
                        {astranaut_quote, {conflicting_quote_context_options, my_ctx, no_context}}}
                ]}
            ],
            []},
        Result
...
test_invalid_context(_Config) ->
    ...
    ?assertMatch(
        {error,
            [
                {_, [
                    {_, astranaut_lib, {astranaut_quote, {invalid_quote_context, {var, _, 'Ctx'}}}}
                ]}
            ],
            _},
        Result
...
test_empty_context(_Config) ->
    ...
    Result = astranaut_quote:parse_transform(Forms, []),
    ?assertMatch(
        {error, [{_, [{_, astranaut_lib, {astranaut_quote, {invalid_quote_context, ''}}}]}], _},
        Result
    ),
    {error, [{_, Errors}], _} = Result,
    astranaut_test_lib:assert_formatted_messages(Errors),
    ?assertError(
        {invalid_quote_context, ''},
...
test_empty_default_context(_Config) ->
    Forms = [
        {attribute, 1, file, {"empty_default_context_test.erl", 1}},
        {attribute, 1, module, ''},
        {function, 2, run, 0, [{clause, 2, [], [], [{call, 2, {atom, 2, quote}, [{var, 2, 'A'}]}]}]},
        {eof, 3}
    ],
    Result = astranaut_quote:parse_transform(Forms, []),
    ?assertMatch(
...
test_low_level_option_validation(_Config) ->
    ...
    ?assertError(
        {invalid_quote_no_context, invalid},
        astranaut_quote:quoted(
            merl:quote(0, "A"), #{no_context => invalid}
        )
    ),
    ?assertError(
        {conflicting_quote_context_options, ctx, no_context},
        astranaut_quote:quoted(
            merl:quote(0, "A"),
