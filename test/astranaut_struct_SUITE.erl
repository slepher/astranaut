...
-include("astranaut_struct_SUITE_data/struct_test_record.hrl").
...
init_per_suite(Config) ->
    erlang:system_flag(backtrace_depth, 500),
...
test_compile_enforce_fail(Config) ->
    {[{File, Errors}], []} = astranaut_test_lib:analyze_module_errors(astranaut_struct_fail_0, yep, Config),
    ?assertEqual([{1,astranaut_struct_transformer, {enforce_keys_not_in_struct,test,[desc]}}], Errors),
    ?assertEqual("astranaut_struct_fail_0.erl", filename:basename(File)),
    ok.

test_compile_non_record_fail(Config) ->
    {[{File, [Error]}], []} = astranaut_test_lib:analyze_module_errors(astranaut_struct_fail_1, yep, Config),
    ?assertMatch({1,astranaut_struct_transformer,{undefined_record,other_test}}, Error),
