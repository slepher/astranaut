...
test_uniform_cross_import_order(_Config) ->
    ?assertEqual(
        {b_generated, {a, {from_a, ok}}},
        macro_uniform_test:later_generates_earlier()
    ),
    ?assertEqual(
        {a_generated, {b, {from_b, ok}}},
...
test_uniform_generated_macro_chain(_Config) ->
    ?assertEqual(
        {a_generated_chain, {b_generated, {a, {from_a, ok}}}},
...
test_uniform_macro_override_error(Config) ->
    ...
    ?assertEqual(
        [{2, astranaut_macro, {missing_macro_formatter, macro_uniform_override_error_test}}],
        Warnings
    ),
...
test_uniform_import_override_error(Config) ->
    ...
    ?assertEqual(
        [{-4, astranaut_macro, {missing_macro_formatter, macro_uniform_b}}],
        Warnings
    ),
    astranaut_test_lib:assert_formatted_messages(Errors),
    ok.
...
test_uniform_external_after_local_error(Config) ->
    assert_macro_error(
        ...
        fun
            (
                {macro_override, {same_name, 1}, #{macro_source := local_macro}, #{
        ...
        [
            {2, astranaut_macro,
                {missing_macro_formatter, macro_uniform_external_after_local_error_test}}
        ]
    ),
    ok.
...
test_uniform_macro_error(Config) ->
    ...
    ?assertEqual(
        [{-2, astranaut_macro, {missing_macro_formatter, macro_uniform_b}}],
        Warnings
    ),
    astranaut_test_lib:assert_formatted_messages(Errors),
    ok.
...
test_uniform_local_macro_invalid_return(Config) ->
    ...
    ?assertEqual(
        [{-2, astranaut_macro, {missing_macro_formatter, macro_uniform_invalid_local_return_test}}],
        Warnings
    ),
...
test_uniform_macro_max_depth(Config) ->
    ...
    ?assertMatch(
        [
            {3, astranaut_macro,
                {max_macro_expansion_depth_exceeded, {macro_uniform_a, recurse_a}, [
                    {integer, _, 12}
                ]}}
        ],
        Errors
