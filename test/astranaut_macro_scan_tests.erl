    ...
    {just,
        ...
        astranaut_return:run(
            astranaut_macro_scan:run(
                ordinary_macro_scan_test,
                "ordinary_macro_scan_test.erl",
                #{max_depth => 100},
                Forms,
                []
            )
...
splice_empty_test() ->
    ...
    false = lists:any(
        fun
            ({attribute, _, import_macro, _}) -> true;
            (_) -> false
        end,
        Result
    ),
...
expand_test() ->
    ...
    Handler = fun
        ({attribute, _Pos, my_macro, _}) ->
            Expanded = [
                {attribute, {10, 1}, local_macro, [{gen, 0}]},
    ...
    true = lists:any(
        fun
            ({attribute, _, local_macro, _}) -> true;
            (_) -> false
        end,
        Result
    ),
...
preserve_generated_function_position_test() ->
    ...
    Generated =
        {function, {10, 1}, generated, 0, [{clause, {10, 1}, [], [], [{atom, {10, 5}, ok}]}]},
    Forms = [
        {attribute, {1, 1}, module, test},
        Before,
...
generated_function_original_merge_test() ->
    ...
    Generated =
        {function, {10, 1}, foo, 0, [
            {clause, {10, 1}, [], [], [{call, {10, 5}, {atom, {10, 5}, '__original__'}, []}]}
        ]},
    Forms = [
        {attribute, {1, 1}, module, test},
        Original,
    ...
    false = astranaut:search(
        fun
            ({call, _, {atom, _, '__original__'}, []}) -> true;
            (_) -> false
        end,
        Result,
        #{traverse => pre}
...
run_original_merge(Prefix, GeneratedForms) ->
    Forms =
        [{attribute, {1, 1}, module, test}] ++ Prefix ++
            [{attribute, {3, 1}, my_macro, ok}],
    Handler = fun
        ({attribute, _Pos, my_macro, _}) ->
...
spec_form(Pos, Name) ->
    {attribute, Pos, spec,
        {{Name, 0}, [{type, Pos, 'fun', [{type, Pos, product, []}, {type, Pos, atom, []}]}]}}.
