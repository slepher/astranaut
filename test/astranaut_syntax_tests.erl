...
test_validate_child_role_error_test() ->
...
test_validate_invalid_node_test() ->
...
test_validate_expression_accept_more_test() ->
    ...
    ?assertEqual(
        ok,
        validate(
            {'case', 1, {atom, 1, x}, [{clause, 1, [{atom, 1, x}], [], [{atom, 1, ok}]}]},
            expression
        )
    ),
...
test_validate_expression_reject_test() ->
...
test_validate_pattern_accept_test() ->
...
test_validate_pattern_reject_test() ->
    {error, #{reason := invalid_role}} = validate(
...
test_validate_guard_accept_test() ->
    ...
    ?assertEqual(
        ok,
        validate(
            {call, 1, {remote, 1, {atom, 1, erlang}, {atom, 1, '+'}}, [
                {var, 1, 'A'}, {var, 1, 'B'}
            ]},
            guard
        )
...
test_validate_guard_reject_test() ->
    {error, #{reason := invalid_role}} = validate(
...
test_validate_function_with_real_guard_test() ->
    Form = parse_form("f(X) when X > 0 -> ok."),
    ?assertEqual(ok, validate(Form, form)).

test_validate_guard_uses_record_forms_test() ->
    GuardRecord = {record, 1, guard_rec, []},
    ValidRecordForms = [
        {attribute, 1, record, {guard_rec, [{record_field, 1, {atom, 1, field}, {integer, 1, 1}}]}}
    ],
    InvalidRecordForms = [
        {attribute, 1, record,
            {guard_rec, [{record_field, 1, {atom, 1, field}, {call, 1, {atom, 1, helper}, []}}]}}
    ],
    ?assertEqual(
        ok,
        validate(
...
test_validate_type_accept_test() ->
...
test_validate_type_and_spec_attributes_test() ->
...
test_reject_malformed_type_and_spec_attributes_test() ->
    MalformedBodies =
        [
            {attribute, 1, type, []},
...
test_validate_clause_accept_test() ->
    ?assertEqual(
        ok,
        validate(
...
test_validate_clause_reject_test() ->
...
test_validate_name_accept_test() ->
    ?assertEqual(ok, validate({atom, 1, foo}, name)).

test_validate_name_reject_test() ->
...
test_validate_attribute_body_accept_test() ->
...
test_generated_schema_dispatch_test() ->
    ?assertEqual(
        astranaut_syntax_schema:node_roles(atom),
    ...
    ?assertEqual(
        {ok, [
            {operator, expression, [Operator], nodes},
...
test_child_specs_include_slot_validators_test() ->
    Pattern = {bin, 1, []},
    Body = {var, 1, 'Bin'},
    Specs = astranaut_syntax:child_specs(binary_generator, [[Pattern], [Body]], #{
        node => expression
    }),
    ?assertMatch(
        [
...
test_all_child_specs_emit_expected_validators_test() ->
    lists:foreach(
        fun({ParentType, Subtrees, Attr, ExpectedSlots}) ->
            Specs = astranaut_syntax:child_specs(ParentType, Subtrees, Attr),
            ActualSlots = [
                {Slot, Role, Validator, maps:with([node, validator], SpecAttr)}
            ...
            Expected =
                [
                    {Slot, Role, {slot, ParentType, Slot, Role},
                        expected_child_attr(
                            Role, Attr, {slot, ParentType, Slot, Role}, ParentType, Slot
                        )}
...
test_all_child_spec_validators_accept_and_reject_ast_test() ->
    Validators =
        lists:append(
            [
                [
                    Validator
                 || #{validator := Validator} <- astranaut_syntax:child_specs(
                        ParentType, Subtrees, Attr
                    )
                ]
...
test_slot_roles_do_not_replace_node_role_test() ->
    ...
    [NameSpec | _] = astranaut_syntax:child_specs(
        function,
        [[valid_ast(name)], [valid_ast(clause)]],
        #{node => form}
    ),
    ?assertNot(maps:is_key(node, maps:get(attr, NameSpec))).

test_binary_field_projection_slots_test() ->
    Value = {var, 1, 'X'},
    Size = {integer, 1, 8},
    ...
    [DefaultValueSpec] =
        astranaut_syntax:child_specs(
            binary_field,
            astranaut_syntax:subtrees(DefaultField),
            #{node => pattern}
        ),
    ?assertMatch(
        #{slot := value, role := pattern, subtrees := [Value]},
        DefaultValueSpec
    ),
    [QualifiedValueSpec, TypesSpec] =
        astranaut_syntax:child_specs(
            binary_field,
            astranaut_syntax:subtrees(SizedTypedField),
            #{node => pattern}
        ),
    ...
    [ValueSpec, SizeSpec] =
        astranaut_syntax:child_specs(
            size_qualifier,
            astranaut_syntax:subtrees(SizeQualifier),
            #{node => pattern}
        ),
    ?assertMatch(
        #{slot := value, role := pattern, subtrees := [Value]},
    ...
    ?assertMatch(
        #{slot := size, role := binary_size, subtrees := [Size]},
        SizeSpec
    ).

test_slot_validators_reject_wrong_structural_identity_test() ->
    BinElement = valid_ast(binary_field),
    MapField = {map_field_exact, 1, {atom, 1, key}, {atom, 1, value}},
    ?assertEqual(
        ok, astranaut_syntax:validate_node(BinElement, {slot, binary, elements, binary_field})
    ),
    ?assertEqual(
        ok,
    ...
    {error, BinError} = astranaut_syntax:validate_node(
        BinElement, {slot, tuple, elements, expression}
    ),
    ?assertMatch(
        #{
            reason := invalid_role,
    ...
    {error, MapError} = astranaut_syntax:validate_node(
        MapField,
        {slot, map_field_exact, map_field_exact_value, expression}
    ),
    ?assertMatch(
...
test_slot_validators_reject_malformed_structural_nodes_test() ->
...
test_validate_node_does_not_recurse_grandchildren_test() ->
    DirectChildInvalid = {call, 1, {atom, 1, f}, [function_form()]},
    GrandchildInvalid =
        {call, 1, {atom, 1, f}, [{tuple, 1, [{clause, 1, [], [], [{atom, 1, ok}]}]}]},
...
test_normalize_recurses_into_grandchildren_test() ->
...
test_try_handler_validation_uses_abstract_format_test() ->
    ...
    ?assertNotEqual(
        astranaut_syntax:subtrees(LegacyHandler),
    ...
    {ok, LegacyHandler1} = astranaut_syntax:normalize(
        LegacyHandler,
        HandlerSlot,
        #{otp_vsn => 21}
    ),
    ...
    StacktraceHandlerWithInvalidBody =
        {clause, 1, [{tuple, 1, [{atom, 1, throw}, {var, 1, 'P'}, {var, 1, 'S'}]}], [], [
            function_form()
        ]},
    ?assertEqual(
        ok,
...
test_legacy_catch_handler_test() ->
...
test_otp21_stacktrace_catch_test() ->
...
test_legacy_map_pattern_key_test() ->
...
test_otp23_map_pattern_key_expression_test() ->
    NewField = {map_field_exact, 1, {op, 1, '+', {var, 1, 'K'}, {integer, 1, 1}}, {var, 1, 'V'}},
    NewCode = "f(#{(K + 1) := V}) -> V.",
...
test_otp23_binary_size_expression_test() ->
    NewBin =
        {bin, 1, [
            {bin_element, 1, {var, 1, 'X'}, {op, 1, '+', {var, 1, 'N'}, {integer, 1, 1}}, default}
        ]},
    NewCode = "f(<<X:(N + 1)>>) -> X.",
    case current_otp_at_least(23) of
        true ->
...
test_otp25_maybe_expr_test() ->
    MaybeExpr =
        {'maybe', 1, [
            {maybe_match, 1, {tuple, 1, [{atom, 1, ok}, {var, 1, 'B'}]}, {var, 1, 'A'}},
            {var, 1, 'B'}
        ]},
...
test_otp26_map_comprehension_test() ->
    MapComp =
        {mc, 1, {map_field_assoc, 1, {var, 1, 'K'}, {var, 1, 'V'}}, [
...
test_otp28_strict_generator_test() ->
...
test_otp29_multiple_comprehension_templates_test() ->
    Generator = {generate, 1, {var, 1, 'I'}, {var, 1, 'List'}},
    SingleListComp = {lc, 1, {var, 1, 'I'}, [Generator]},
    MultiListComp =
        {lc, 1, [{var, 1, 'I'}, {op, 1, '-', {var, 1, 'I'}}], [Generator]},
    SingleMapComp =
        {mc, 1, {map_field_assoc, 1, {var, 1, 'I'}, {var, 1, 'I'}}, [Generator]},
    MultiMapComp =
        {mc, 1,
            [
                {map_field_assoc, 1, {var, 1, 'I'}, {var, 1, 'I'}},
                {map_field_assoc, 1, {op, 1, '+', {var, 1, 'I'}, {integer, 1, 1}}, {var, 1, 'I'}}
            ],
            [Generator]},
    ?assertEqual(ok, validate(SingleListComp, expression, #{otp_vsn => 28})),
    ?assertMatch(
        {error, #{reason := invalid_role, actual_type := list_comp}},
...
test_traverse_validate_changed_node_test() ->
    Tree = {call, 1, {atom, 1, f}, [{atom, 1, bad}]},
    try
        astranaut:smap(
            fun
                ({atom, 1, bad}) -> function_form();
                (Node) -> Node
            end,
            Tree,
            #{traverse => pre, role => expression, validate => true}
        )
    of
        _ -> error(unexpected_ok)
    catch
        error:{invalid_transform_normalization, #{
            reason := invalid_role,
            actual_type := function
        }} ->
            ok
...
test_traverse_validate_false_opt_out_test() ->
    Tree = {call, 1, {atom, 1, f}, [{atom, 1, bad}]},
    ?assertEqual(
        ...
        astranaut:smap(
            ...
            Tree,
            #{traverse => pre, validate => false}
        )
    ).

test_traverse_validate_preserves_slot_attr_test() ->
    Tree =
        {'case', 1, {var, 1, 'M'}, [
            {clause, 1, [{map, 1, [{map_field_exact, 1, {atom, 1, key}, {var, 1, 'V'}}]}], [], [
                {atom, 1, ok}
            ]}
        ]},
    ...
    try
        astranaut:smap(
            fun
                ({map_field_exact, 1, {atom, 1, key}, {var, 1, 'V'}}, _Attr) ->
                    InvalidField;
                (Node, Attr) ->
            ...
            Tree,
            #{traverse => pre, role => expression, validate => output}
        )
    of
        _ -> error(unexpected_ok)
    catch
...
test_traverse_pre_validate_does_not_recurse_grandchildren_test() ->
    Tree = {call, 1, {atom, 1, f}, [{atom, 1, bad}]},
...
test_traverse_post_validate_recurse_grandchildren_test() ->
    Tree = {call, 1, {atom, 1, f}, [{atom, 1, bad}]},
    Updated = {tuple, 1, [{tuple, 1, [{clause, 1, [], [], [{atom, 1, ok}]}]}]},
    try
        astranaut:smap(
            ...
            Tree,
            #{traverse => post, role => expression, validate => output}
        )
    of
        _ -> error(unexpected_ok)
    catch
...
test_traverse_input_validate_runs_before_post_walk_test() ->
    Tree = {call, 1, {atom, 1, f}, [{clause, 1, [], [], [{atom, 1, ok}]}]},
    try
        astranaut:smap(
            fun(Node) -> Node end,
            Tree,
            #{traverse => post, role => expression, validate => input}
        )
    of
        _ -> error(unexpected_ok)
    catch
...
test_traverse_input_validate_attribute_body_test() ->
    ...
    ?assertEqual(
        ...
        astranaut:smap(
            fun(Node) -> Node end,
            Attribute,
            #{traverse => post, role => form, validate => input}
        )
    ).

test_traverse_validate_collect_returns_input_test() ->
    ...
    ?assertMatch(
        [{invalid_transform_normalization, #{reason := invalid_role, actual_type := clause}}],
        Errors
    ).

test_traverse_true_validate_is_output_test() ->
    Tree = {call, 1, {atom, 1, f}, [{clause, 1, [], [], [{atom, 1, ok}]}]},
    ?assertEqual(
        Tree,
        astranaut:smap(
            fun(Node) -> Node end,
            Tree,
            #{traverse => post, role => expression, validate => true}
        )
    ).

test_traverse_both_validate_includes_input_test() ->
    Tree = {call, 1, {atom, 1, f}, [{clause, 1, [], [], [{atom, 1, ok}]}]},
    try
        astranaut:smap(
            fun(Node) -> Node end,
            Tree,
            #{traverse => post, role => expression, validate => both}
        )
    of
        _ -> error(unexpected_ok)
    catch
...
test_read_only_traversals_skip_validation_test() ->
    Tree = {call, 1, {atom, 1, f}, [{clause, 1, [], [], [{atom, 1, ok}]}]},
    ?assertEqual(
        4,
...
test_map_m_validate_raises_test() ->
    Tree = {call, 1, {atom, 1, f}, [{atom, 1, bad}]},
    Monad =
        astranaut:map_m(
            fun
...
test_set_pos_warning_keeps_shape_test() ->
    ?assertEqual(
        {warning, {2, erl_lint, bad}},
        astranaut_syntax:set_pos({warning, {1, erl_lint, bad}}, 2)
    ).

test_syntax_adapter_helpers_test() ->
    ?assertEqual(
...
test_validate_nested_valid_test() ->
    Tree =
...
test_validate_nested_invalid_test() ->
    Tree = {call, 1, {atom, 1, f}, [{clause, 1, [], [], [{atom, 1, ok}]}]},
...
test_otp29_native_record_forms_test() ->
    ...
    NativeRecord = {attribute, 1, native_record, {rec, [{record_field, 1, {atom, 1, x}}]}},
    ...
    [ArgumentSpec, TypeSpec, FieldsSpec] =
        astranaut_syntax:child_specs(
            record_expr,
            astranaut_syntax:subtrees(QualifiedUpdate),
            #{node => expression}
        ),
    ?assertMatch(
        #{
...
test_otp29_native_record_version_test() ->
    Field = {record_field, 1, {atom, 1, x}, {integer, 1, 1}},
    QualifiedCreate = {record, 1, {mod, rec}, [Field]},
    AnonymousUpdate = {record, 1, {var, 1, 'R'}, [], [Field]},
    NativeRecord = {attribute, 1, native_record, {rec, [{record_field, 1, {atom, 1, x}}]}},
...
test_record_access_child_slot_order_test() ->
    Argument = [{var, 1, 'R'}],
    Type = [{atom, 1, rec}],
    Field = [{atom, 1, x}],
...
test_attribute_adapter_roundtrip_test() ->
    Forms =
        [
...
test_record_field_adapter_roundtrip_test() ->
    Nodes =
        [
            {record_field, 1, {atom, 1, field}},
            {record_field, 2, {atom, 2, field}, {integer, 2, 42}},
            {typed_record_field, {record_field, 3, {atom, 3, field}, {integer, 3, 0}},
                {type, 3, integer, []}}
        ],
...
test_adapter_slot_replacement_test() ->
    ...
    assert_same_ast(
        {typed_record_field, {record_field, 1, {atom, 1, field}}, NewFieldType},
        UpdatedTypedField
    ),
...
assert_same_ast(Expected, Actual) ->
    ?assertEqual(
        astranaut_lib:replace_pos(Expected, 0),
...
function_body_expr(
    {function, _Pos, _Name, _Arity, [{clause, _ClausePos, _Patterns, _Guards, [Expr]}]}
) ->
    Expr.
...
function_first_pattern(
    {function, _Pos, _Name, _Arity, [{clause, _ClausePos, [Pattern | _Patterns], _Guards, _Body}]}
) ->
    Pattern.
...
stacktrace_catch_try_ast() ->
    Handler =
        {clause, 1, [{tuple, 1, [{atom, 1, error}, {var, 1, 'Reason'}, {var, 1, 'Stacktrace'}]}],
            [], [{var, 1, 'Reason'}]},
    {'try', 1, [{atom, 1, body}], [], [Handler], []}.

legacy_throw_handler_ast() ->
    {clause, 1, [{tuple, 1, [{atom, 1, throw}, {var, 1, 'P'}, {var, 1, '_'}]}], [], [{var, 1, 'P'}]}.
...
stacktrace_throw_handler_ast() ->
    {clause, 1, [{tuple, 1, [{atom, 1, throw}, {var, 1, 'P'}, {var, 1, 'S'}]}], [], [{var, 1, 'P'}]}.
...
child_spec_cases() ->
    ...
    [
        ...
        {clause, [[Pattern], [Expr]], #{node => clause}, [{patterns, pattern}, {body, expression}]},
        {clause, [[Pattern], [Guard], [Expr]], #{node => clause}, [
            {patterns, pattern}, {guards, guard}, {body, expression}
        ]},
        {binary, [[valid_ast(binary_field)]], #{node => expression}, [{elements, binary_field}]},
        {map_expr, [[MapField]], #{node => expression}, [{fields, map_field}]},
        {map_expr, [[Expr], [MapField]], #{node => expression}, [
            {argument, expression}, {fields, map_field}
        ]},
        {map_expr, [[MapField]], #{node => pattern}, [{fields, map_field}]},
        ...
        {fun_expr, [[Clause]], #{node => expression}, [{clauses, clause}]},
        {case_expr, [[Expr], [Clause]], #{node => expression}, [
            {argument, expression}, {clauses, clause}
        ]},
        {if_expr, [[Clause]], #{node => expression}, [{clauses, clause}]},
        {receive_expr, [[Clause]], #{node => expression}, [{clauses, clause}]},
        ...
        {function, [[Name], [Clause]], #{node => form}, [{name, name}, {clauses, clause}]},
        {function, [[Clause]], #{node => form}, [{clauses, clause}]},
        ...
        {maybe_expr, [[Expr]], #{node => expression}, [{body, expression}]},
        {maybe_expr, [[Expr], [Clause]], #{node => expression}, [
            {body, expression}, {else_clause, clause}
        ]},
        {implicit_fun, [[Expr]], #{node => expression}, [{name, expression}]},
        {record_access, [[Expr], [Expr], [Expr]], #{node => expression}, [
            {argument, expression}, {type, expression}, {field, expression}
        ]},
        {zip_generator, [[Expr]], #{node => expression}, [{body, expression}]},
        {tuple, [[Pattern]], #{node => pattern}, [{elements, pattern}]},
        {tuple, [[Expr]], #{node => expression}, [{elements, expression}]},
        {tuple, [[Expr]], #{}, [{elements, expression}]}
    ].
