...
-type rtrees() ::
    astranaut_uniplate:node_context(tree()) | [astranaut_uniplate:node_context(tree())].
...
-type common_walk_return(State, Value) ::
    astranaut_traverse:struct(State, Value)
    | astranaut_return:struct(Value)
    | walk_return(State, Value)
    | walk_return_tuple(Value).
...
-type walk_return_tuple(A) ::
    convertable_warning(A)
    | convertable_error(A)
    | continue
    | {continue, A}
    | {node, A | [A]}.

-type convertable_warning(A) ::
    {warning, error_term()}
    | {warnings, [error_term()]}
    | {warning, A, error_term()}
    | {warnings, A, [error_term()]}.

-type convertable_error(A) ::
    {error, error_term()}
    | {errors, [error_term()]}
    | {error, A, error_term()}
    | {errors, A, [error_term()]}.
...
-spec smap(fun((tree()) -> rtrees()) | fun((tree(), #{}) -> rtrees()), trees(), straverse_opts()) ->
    trees().
smap(F, Node, Opts) ->
...
-spec sreduce(fun((tree(), S) -> S) | fun((tree(), S, #{}) -> S), S, trees(), straverse_opts()) ->
    S.
sreduce(F, Init, Node, Opts) ->
    F1 = fun(N, A) -> fun(S) -> {N, apply_fun(F, [N, S, A])} end end,
    StateM = map_m_with_attr(
        F1, Node, state, Opts#{static => true, normalize => false}, is_function(F, 3)
    ),
...
-spec smap_with_state(
    fun((tree(), S) -> {rtrees(), S}) | fun((tree(), S, #{}) -> {rtrees(), S}),
    S,
    trees(),
    straverse_opts()
) -> trees().
...
-spec search(fun((N) -> boolean()) | fun((N, map()) -> boolean()), N, straverse_opts()) ->
    boolean().
search(F, Node, Opts) ->
    F1 = fun(N, A) ->
    ...
    Either = map_m_with_attr(
        F1, Node, either, Opts#{static => true, normalize => false}, is_function(F, 2)
    ),
    case Either of
        {left, match} -> true;
        {right, _Node} -> false
...
-spec map_with_state(mapfold_walk(S), S, trees(), traverse_opts()) ->
    astranaut_return:struct(trees()).
...
-spec mapfold(mapfold_walk(S), S, trees(), traverse_opts()) ->
    astranaut_return:struct({trees(), S}).
        ...
        astranaut_traverse:map_m(
            fun(Form) ->
                astranaut_traverse:lift_m(
                    ...
                    astranaut_traverse:listen_updated(
...
validation_attr_needed(_Node, #{validate := Validate}) when
    Validate =:= true; Validate =:= input; Validate =:= output; Validate =:= both
->
    true;
...
format_error({validate_key_failure, {invalid_validator_arg, Validator}, Key, _Value}) when
    is_atom(Validator)
->
...
format_error({validate_key_failure, {invalid_validator_return, Validator, Return}, Key, _Value}) ->
    io_lib:format("validator ~p for option key ~p returns a invalid_value ~p", [
        Validator, Key, Return
    ]);
