...
function_ids(Forms, Functions0) ->
    lists:foldl(
        fun
            ({function, _Pos, Name, Arity, _Clauses}, Functions) ->
...
append_form(
    {attribute, _Pos, module, _Name} = Module,
    #grouped_forms{
        eof = [],
        functions = [],
        attributes = [],
...
append_form(
    {attribute, _Pos, file, _Value} = File,
    #grouped_forms{
        eof = [],
        functions = [],
        attributes = [],
...
insert_attribute(
    ...
    #grouped_forms{
        module =
            [{attribute, _Pos, module, _Name} | ModuleForms]
    } = Groups
) ->
    Groups#grouped_forms{module = [Module | ModuleForms]};
insert_attribute(
...
merge_functions(NewForms, NewFunctions, Functions, Groups, Tails) ->
    ...
    {Functions2, NewFormsReversed, Groups1, Tails1} =
        lists:foldl(
            fun
                (
                    {function, _Pos, Name, Arity, _Clauses} = Form,
                    {FunctionsAcc, NewFormsAcc, GroupsAcc, TailsAcc}
                ) ->
                    case
                        ordsets:is_element({Name, Arity}, ExistingNewFunctions) andalso
...
is_renamed(Arity, Form) ->
    astranaut:search(
        fun
            ({call, _Pos1, {atom, _Pos2, '__original__'}, Arguments}) ->
...
update_function_name(Name, Arity, NewName, Forms) ->
    lists:map(
        fun
            ({function, Pos, FunctionName, FunctionArity, Clauses}) when
                FunctionName =:= Name, FunctionArity =:= Arity
            ->
...
update_call_name(OriginalName, NewName, Arity, Tree) ->
    astranaut:smap(
        fun
            ({call, Pos, {atom, NamePos, Name}, Arguments}) when
                Name =:= OriginalName, length(Arguments) =:= Arity
            ->
