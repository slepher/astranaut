...
do({lc, Pos, Monad, Comprehensions}, Opts) ->
    astranaut_return:lift_m(
        ...
        do_comprehensions(Comprehensions, Monad, Opts)
    );
do(_Ast, #{}) ->
...
do_comprehensions([{GenerateOrMatch, _Pos, _Pattern, _Expr}], _Monad, #{}) when
...
bind_expression(Expr, Exprs, Monad, #{monad := MonadModule}) ->
    ...
    [
        quote(
            _A@MonadModule:bind(
                unquote(Expr), fun(_) -> unquote_splicing(Exprs) end, unquote(Monad)
            ),
            Pos
        )
    ].
