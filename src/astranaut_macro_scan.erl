...
map_forms_splice_result(
    {buffer, NewForms, MaxDepth, Origin},
    _Tag,
    F,
    Rest,
    _Buffer,
...
map_forms_splice_result(
    ...
    map_forms_splice_loop(
        ...
        Rest,
        Buffer,
        [map_forms_splice_tag_result(Tag, Form1) | Acc],
        Opts
    ).
...
map_forms_splice_merge_specs(TaggedForms) ->
    ...
    GeneratedSpecFAs = ordsets:from_list(
        [
            FA
         || {generated_insert, Form} <- TaggedForms,
...
map_forms_splice_reorder(
    [{generated_insert, {function, _Pos, Name, Arity, _Clauses} = Form} | T], Acc
) ->
...
map_forms_splice_forms_functions(Forms) ->
    lists:foldl(
        fun
            ({function, _Pos, Name, Arity, _Clauses}, Acc) ->
...
map_forms_splice_update_call_name(
    ...
    astranaut:smap(
        fun
            ({call, Pos, {atom, Pos2, Name}, Arguments}) when
...
scan_env_form(Form, #{registry := Registry} = State) ->
    do([
     ...
     || {Action, Registry1} <- astranaut:traverse_return(
            astranaut_macro_registry:apply_directive(
                Form, Registry
            )
        ),
...
scan_attribute_runtime(
    ...
    case
        astranaut_macro_expander:resolve_attribute_target(
            Form, MacroMap, AttributeIndex
        )
    of
        {ok, Target0} ->
...
attribute_expansion_limit_error(
    ...
    {max_macro_expansion_depth_exceeded, maps:get(macro, Macro), Arguments}.
