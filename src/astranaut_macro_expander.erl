...
expand_functions(Forms, Tasks) ->
    ...
    Monad =
        astranaut:map_m(
            fun
                ({function, _Pos, Name, Arity, _Clauses} = Function) ->
...
function_call_analysis(Forms, MacroMap, Mode) ->
    lists:foldl(
        fun
            ({function, _Pos, Name, Arity, Clauses} = Form, Acc) ->
...
function_macro_callers(FunctionAnalysis) ->
    maps:fold(
        fun
            (FormId, #{has_macro_call := true}, Acc) ->
...
function_has_macro_call(_Clauses, MacroMap, _Hint) when
    map_size(MacroMap) =:= 0
->
    false;
...
initial_expansion_state(disabled) ->
    0;
initial_expansion_state(#{mode := Mode}) when
    Mode =:= collect; Mode =:= verify
->
    #{
...
call_find_macro(
    _Module,
    {call, Pos1, {remote, _Pos2, {atom, _Pos3, RemoteModule}, {atom, _Pos4, Function}}, Arguments} =
        Node,
    Macros
) ->
...
match_macro_order(Macro, Step) ->
    ...
    ((Order =:= inner) and (Step =:= post)) or
        ((Order =:= outer) and (Step =:= pre)).

expand_macro_recursive(
    _Module,
    _MacroMap,
    #{max_depth := MaxDepth} = Macro,
...
->
    ...
    recover_macro_call(
        ...
        astranaut_traverse:fail(
            {max_macro_expansion_depth_exceeded, maps:get(macro, CurrentMacro),
...
expand_macro_recursive(
    ...
    expand_macro_with(
        Macro,
        DepthOpts1#{module => Module, macro_map => MacroMap},
        fun(Node1) ->
    ...
    recover_macro_call(
        ...
        do([
         ...
         || Node <- astranaut_traverse:update_pos(
            ...
            case {Decision, maps:get(has_macro_call, ReturnAnalysis)} of
     ...
     || Attr <- astranaut_traverse:ask(),
        ...
        ProcessReturn =
            astranaut_traverse:with_all_error(
                fun
                    ({invalid_transform_normalization, Detail}) ->
...
resolve_attribute_target_by_name(
    ...
    case maps:find(Attribute, AttributeIndex) of
        ...
        {ok, AttributeMacros} ->
            ...
            case
                find_attribute_macro(
                    Attribute, length(RawArguments), AttributeMacros
                )
            of
                {ok, Macro} ->
                    {ok, #{
...
analyze_function_calls(Clauses, MacroMap, closure) ->
    astranaut:sreduce(
        fun
            (
                {call, _Pos, {atom, _FunctionPos, Function}, Arguments} = Node,
                Analysis
            ) ->
                analyze_macro_call(
                    Node,
            ...
            (
                {call, _Pos,
                    {remote, _RemotePos, {atom, _ModulePos, _Module},
                        {atom, _FunctionPos, _Function}},
                    _Arguments} = Node,
                Analysis
            ) ->
