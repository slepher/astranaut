...
module_macro_maps_from_uniform(MacroMap) ->
    maps:fold(
      fun(Key, #{macro_module := MacroModule} = Macro, Acc) ->
              ...
              maps:put(
                MacroModule,
                maps:put(Key, Macro, ModuleMacroMap), Acc)
      end, #{}, MacroMap).

import_macro_form(
  GlobalMacroOpts,
  {attribute, _Pos, import_macro, Module}) when is_atom(Module) ->
...
update_module_macros(File, Module, ModuleMacros) ->
    maps:fold(
      fun(_MFA, MacroOptions, Acc) ->
              MacroOptions1 = MacroOptions#{file => File,
                                             local_module => Module},
...
used_macros(File, Module, ImportedMacros, Forms) ->
    ...
    astranaut_lib:with_attribute(
      fun(Attr, UsedMacroMapAcc) ->
              do([ return ||
                     ...
                     {MFAs, Options} <- validate_macro_attribute(
                                          fun macro_attr/1,
                                          Validator, use_macro, Attr),
                     case MFAs of
                         {ImportedModule, FAs} ->
                             case maps:is_key(
                                    ImportedModule, UsedMacroMapAcc) of
                                 true ->
                                     update_used_macro_maps(
                                       File, Module, ImportedModule,
                                       FAs, Options, UsedMacroMapAcc,
                                       fun({Function, Arity}) ->
                                               {unexported_macro,
                                                ImportedModule,
                                                Function, Arity}
      ...
      end, ImportedMacroMap, Forms, use_macro,
...
find_used_macro({Function, Arity}, ModuleMacroMap) ->
    maps:fold(
      fun(MacroKey,
          ...
          error) when Function =:= Function1, Arity =:= Arity1 ->
              {ok, MacroKey, MacroOptions};
...
analyze_module_macros(Module) ->
    ...
    lists:foldl(
      fun({FAs, Opts}, Acc) -> Insert(FAs, Opts, Acc);
...
macro_attr({Module, FA}) when is_atom(Module), not is_integer(FA) ->
    {{Module, [FA]}, []};
macro_attr({Module, FAs, Options})
  when is_atom(Module), is_list(FAs) ->
    {{Module, FAs}, Options};
...
merge_macro_maps(First, Second) ->
    case merge_macro_maps_pure(First, Second) of
        ...
        {error, {macro_override, MacroKey,
                 ExistingMacro, OverridingMacro}} ->
            macro_override_fail(
