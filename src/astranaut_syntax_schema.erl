...
format_bounds(annotated_type, {ann_type, _, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(application, {call, _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
...
format_bounds(binary, {bin, _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(binary_comp, {bc, _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
format_bounds(binary_field, {bin_element, _, Node1, default, default}) when
    not is_list(Node1)
->
    {21, 29};
format_bounds(binary_field, {bin_element, _, Node1, Node2, default}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(binary_field, {bin_element, _, Node1, default, Values2}) when
    not is_list(Node1), is_list(Values2)
->
    {21, 29};
format_bounds(binary_field, {bin_element, _, Node1, Node2, Values3}) when
    not is_list(Node1), not is_list(Node2), is_list(Values3)
->
    {21, 29};
format_bounds(binary_generator, {b_generate, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(bitstring_type, {type, _, binary, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(block_expr, {block, _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(case_expr, {'case', _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
...
format_bounds(char, {char, _, _}) ->
    {21, 29};
format_bounds(clause, {clause, _, Nodes1, Nodes2, Nodes3}) when
    is_list(Nodes1), is_list(Nodes2), is_list(Nodes3)
->
    {21, 29};
format_bounds(cons, {cons, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(
    ...
    is_list(Nodes2), not is_list(Node1)
->
    {21, 29};
format_bounds(constraint, {type, _, constraint, [Node1, Nodes2]}) when
    is_list(Nodes2), not is_list(Node1)
->
    {21, 29};
...
format_bounds(float, {float, _, _}) ->
    {21, 29};
format_bounds(fun_expr, {'fun', _, {clauses, Nodes1}}) when
    is_list(Nodes1)
->
    {21, 29};
format_bounds(fun_type, {type, _, 'fun', []}) ->
    {21, 29};
format_bounds(function, {function, _, _, _, Nodes1}) when
    is_list(Nodes1)
->
    {21, 29};
format_bounds(
    ...
    not is_list(Node2), is_list(Nodes1)
->
    {21, 29};
format_bounds(function_type, {type, _, 'fun', [{type, _, any}, Node1]}) when
    not is_list(Node1)
->
    {21, 29};
format_bounds(generator, {generate, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
...
format_bounds(implicit_fun, {'fun', _, {function, _, _}}) ->
    {21, 29};
format_bounds(implicit_fun, {'fun', _, {function, Node1, Node2, Node3}}) when
    not is_list(Node3), not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(infix_expr, {op, _, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(integer, {integer, _, _}) ->
    {21, 29};
format_bounds(integer_range_type, {type, _, range, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(list, {cons, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(list_comp, {lc, _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
format_bounds(list_comp, {lc, _, Nodes1, Nodes2}) when
    is_list(Nodes1), is_list(Nodes2)
->
    {29, 29};
format_bounds(map_comp, {mc, _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
format_bounds(map_comp, {mc, _, Nodes1, Nodes2}) when
    is_list(Nodes1), is_list(Nodes2)
->
    {29, 29};
format_bounds(map_expr, {map, _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(map_expr, {map, _, Node1, Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {21, 29};
format_bounds(map_field_assoc, {map_field_assoc, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(map_field_exact, {map_field_exact, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(map_generator, {m_generate, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
...
->
    {21, 29};
format_bounds(match_expr, {match, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(maybe_expr, {'maybe', _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(maybe_expr, {'maybe', _, Nodes1, {'else', _, Nodes2}}) when
    is_list(Nodes1), is_list(Nodes2)
->
    {21, 29};
format_bounds(maybe_match_expr, {maybe_match, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(module_qualifier, {remote, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(named_fun_expr, {named_fun, _, _, Nodes1}) when
    is_list(Nodes1)
->
    {21, 29};
...
format_bounds(receive_expr, {'receive', _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(receive_expr, {'receive', _, Nodes1, Node2, Nodes3}) when
    is_list(Nodes1), not is_list(Node2), is_list(Nodes3)
->
    {21, 29};
format_bounds(
    record_access,
    ...
    not is_list(Node1), is_atom(LocalRecord2), not is_list(Node3)
->
    {21, 29};
format_bounds(
    record_access,
...
->
    {29, 29};
format_bounds(record_access, {record_field, _, Node1, [], Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {29, 29};
format_bounds(record_expr, {record, _, LocalRecord1, Nodes2}) when
    is_atom(LocalRecord1), is_list(Nodes2)
->
    {21, 29};
format_bounds(record_expr, {record, _, Node1, LocalRecord2, Nodes3}) when
    not is_list(Node1), is_atom(LocalRecord2), is_list(Nodes3)
->
    {21, 29};
format_bounds(record_expr, {record, _, {Module1, Record2}, Nodes3}) when
    is_atom(Record2), is_atom(Module1), is_list(Nodes3)
->
    {29, 29};
...
->
    {29, 29};
format_bounds(record_expr, {record, _, Node1, [], Nodes2}) when
    not is_list(Node1), is_list(Nodes2)
->
    {29, 29};
format_bounds(record_field, {record_field, _, Node1}) when
    not is_list(Node1)
->
    {21, 29};
format_bounds(record_field, {record_field, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(record_index_expr, {record_index, _, _, Node1}) when
    not is_list(Node1)
->
    {21, 29};
format_bounds(record_type, {type, _, record, [Node1 | Nodes2]}) when
    is_list(Nodes2), not is_list(Node1)
->
    {21, 29};
format_bounds(record_type_field, {type, _, field_type, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(remote_type, {type, _, _, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(remote_type, {remote_type, _, [Node1, Node2, Nodes3]}) when
    is_list(Nodes3), not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(
...
->
    {21, 29};
format_bounds(strict_generator, {generate_strict, _, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(
...
format_bounds(string, {string, _, _}) ->
    {21, 29};
format_bounds(try_expr, {'try', _, Nodes1, Nodes2, Nodes3, Nodes4}) when
    is_list(Nodes1),
    is_list(Nodes2),
    is_list(Nodes3),
    is_list(Nodes4)
...
format_bounds(tuple_type, {type, _, tuple, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(type_application, {type, _, _, Nodes1}) when
    is_list(Nodes1)
->
    {21, 29};
format_bounds(
...
->
    {21, 29};
format_bounds(type_binary, {type, _, binary, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
format_bounds(type_fun, {type, _, 'fun', []}) ->
    {21, 29};
format_bounds(type_integer_range, {type, _, range, [Node1, Node2]}) when
    not is_list(Node2), not is_list(Node1)
->
    {21, 29};
...
->
    {21, 29};
format_bounds(type_record, {type, _, record, [Node1 | Nodes2]}) when
    is_list(Nodes2), not is_list(Node1)
->
    {21, 29};
...
format_bounds(type_union, {type, _, union, Nodes1}) when is_list(Nodes1) ->
    {21, 29};
format_bounds(typed_record_field, {typed_record_field, Node1, Node2}) when
    not is_list(Node1), not is_list(Node2)
->
    {21, 29};
format_bounds(underscore, {var, _, '_'}) ->
    {21, 29};
format_bounds(user_type_application, {user_type, _, _, Nodes1}) when
    is_list(Nodes1)
->
    {21, 29};
...
->
    false;
slot_available(size_qualifier, size, default, _ChildNode, OtpVsn) when
    OtpVsn =< 22
->
    true;
slot_available(size_qualifier, size, integer, _ChildNode, OtpVsn) when
    OtpVsn =< 22
->
    true;
slot_available(size_qualifier, size, variable, _ChildNode, OtpVsn) when
    OtpVsn =< 22
->
    true;
slot_available(size_qualifier, size, _ChildType, _ChildNode, OtpVsn) when
    OtpVsn =< 22
->
    false;
...
slot_available(_ParentType, _Slot, _ChildType, _ChildNode, _OtpVsn) ->
    true.
child_layout(attribute, [[NameTree], BodyTrees], _ParentRole, OtpVsn) when
    is_list(BodyTrees)
->
...
child_layout(annotated_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(application, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
...
child_layout(arity_qualifier, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(atom, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(binary, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{elements, binary_field, Group1, nodes}]};
child_layout(binary_comp, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
...
child_layout(binary_field, [Group1 = [_], Group2], ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
...
child_layout(bitstring_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(block_expr, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(case_expr, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
...
child_layout(catch_expr, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(char, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(class_qualifier, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(clause, [Group1, Group2], _ParentRole, _OtpVsn) when
    is_list(Group1), is_list(Group2)
->
    {ok, [
        {patterns, pattern, Group1, nodes},
        {body, expression, Group2, nodes}
    ]};
child_layout(clause, [Group1, Group2, Group3], _ParentRole, _OtpVsn) when
    is_list(Group1), is_list(Group2), is_list(Group3)
->
    {ok, [
        {patterns, pattern, Group1, nodes},
...
child_layout(comment, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(cond_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{clauses, clause, Group1, nodes}]};
child_layout(conjunction, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(cons, Subtrees, ParentRole, _OtpVsn) when is_list(Subtrees) ->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(constrained_function_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(constraint, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(disjunction, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(else_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{clauses, clause, Group1, nodes}]};
...
child_layout(float, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(fun_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{clauses, clause, Group1, nodes}]};
child_layout(fun_type, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(function, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [{name, name, Group1, nodes}, {clauses, clause, Group2, nodes}]};
child_layout(function_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(
...
child_layout(if_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{clauses, clause, Group1, nodes}]};
child_layout(implicit_fun, [Group1 = [_]], _ParentRole, _OtpVsn) ->
    {ok, [{name, expression, Group1, nodes}]};
child_layout(infix_expr, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(integer, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(integer_range_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(list, Subtrees, ParentRole, _OtpVsn) when is_list(Subtrees) ->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(list_comp, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
        {template, expression, Group1, nodes},
...
child_layout(list_comp, [Group1, Group2], _ParentRole, OtpVsn) when
    is_list(Group1), is_list(Group2), OtpVsn >= 29
->
    {ok, [
        {template, expression, Group1, nodes},
        {body, expression, Group2, nodes}
    ]};
child_layout(map_comp, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
...
child_layout(map_comp, [Group1, Group2], _ParentRole, OtpVsn) when
    is_list(Group1), is_list(Group2), OtpVsn >= 29
->
    {ok, [
        {template, map_field, Group1, nodes},
...
child_layout(map_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{fields, map_field, Group1, nodes}]};
child_layout(map_expr, [Group1 = [_], Group2], _ParentRole, _OtpVsn) when
    is_list(Group2)
->
    {ok, [
        {argument, expression, Group1, nodes},
...
child_layout(map_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(map_type_assoc, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(map_type_exact, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
...
child_layout(maybe_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{body, expression, Group1, nodes}]};
child_layout(maybe_expr, [Group1, Group2], _ParentRole, _OtpVsn) when
    is_list(Group1), is_list(Group2)
->
    {ok, [
...
child_layout(module_qualifier, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(
...
child_layout(operator, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(parentheses, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(prefix_expr, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(receive_expr, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{clauses, clause, Group1, nodes}]};
child_layout(
...
child_layout(record_field, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(record_index_expr, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(record_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(record_type_field, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(remote_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(
...
child_layout(tuple, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(tuple_type, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_application, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_binary, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_fun, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(type_integer_range, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_map, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_map_field, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_record, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_tuple, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(type_union, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(typed_record_field, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
child_layout(underscore, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(user_type_application, Subtrees, ParentRole, _OtpVsn) when
    is_list(Subtrees)
->
    {ok, [{elements, ParentRole, Subtrees, groups}]};
...
child_layout(warning_marker, [], _ParentRole, _OtpVsn) ->
    {ok, []};
child_layout(zip_generator, [Group1], _ParentRole, _OtpVsn) when
    is_list(Group1)
->
    {ok, [{body, expression, Group1, nodes}]};
...
attribute_layout(export_record, NameTree, Group1, OtpVsn) when
    OtpVsn >= 29
->
    {ok, [
        {name, name, [NameTree], nodes},
...
attribute_layout(import_record, NameTree, Group1, OtpVsn) when
    OtpVsn >= 29
->
    {ok, [
        {name, name, [NameTree], nodes},
...
attribute_layout(native_record, NameTree, Group1, OtpVsn) when
    OtpVsn >= 29
->
    {ok, [
        {name, name, [NameTree], nodes},
