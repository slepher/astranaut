    ...
    Forms1 = 
        reduce(
          fun({error, Errors}, FormsAcc, #{}) when is_list(Errors) ->
                  astranaut_traverse_m_v3:then(
                    astranaut_traverse_m_v3:put(FormsAcc),
                    astranaut_traverse_m_v3:formatted_errors(Errors));
             (Form, FormsAcc, #{}) ->
                  astranaut_traverse_m_v3:put([Form|FormsAcc])
...
add_attribute(F, AttrValues, Acc) when is_list(AttrValues) ->
    astranaut_traverse_m_v3:then(
      astranaut_traverse_m_v3:put(Acc),
      astranaut_monad:map_m(
        fun(AttrValue) ->
                astranaut_traverse_m_v3:bind(
                  astranaut_traverse_m_v3:get(),
                  fun(Acc1) ->
...
mapfold_1(F, Init, Node, #{} = Opts) ->
    ...
    astranaut_traverse_m_v3:run(NodeM, Formatter, Init).

map_m_1(F, Nodes, Opts) when is_list(Nodes) ->
    astranaut_traverse_m_v3:sequence_either(
      lists:map(
        fun(Subtree) ->
...
map_m_1(F, NodeA, #{traverse := list} = Opts) ->
    ...
    MA = 
        case SyntaxLib:is_file(NodeA) of
            false ->
                astranaut_traverse_m_v3:return(ok);
            {file, File} ->
                astranaut_traverse_m_v3:update_file(File)
        end,
    astranaut_traverse_m_v3:then(
      MA,
...
map_m_tree(_F, {error, Reason}, _Opts, _SyntaxLib) ->
    astranaut_traverse_m_v3:bind(
      astranaut_traverse_m_v3:error(Reason),
      astranaut_traverse_m_v3:nodes([]));
map_m_tree(F, NodeA, Opts, SyntaxLib) ->
            ...
            astranaut_traverse_m_v3:bind_node(
              ...
              fun(NodeB) ->
                      case SyntaxLib:tps(NodeB, Opts) of
                          {_Type, _Pos, []} ->
                              astranaut_traverse_m_v3:nodes([NodeB]);
                              ...
                              astranaut_traverse_m_v3:bind_node(
                                ...
                                fun(NodeC) ->
                                        astranaut_traverse_m_v3:bind_node(
                                          ...
                                          fun astranaut_traverse_m_v3:return/1, post)
                                end, children)
                      end
              end, pre);
        {file, File} ->
...
apply_f(F, Node, Attr, SyntaxLib) ->
    ...
    astranaut_traverse_m_v3:update_line(Line, F(Node, Attr)).
...
map_m_children(_F, Node, [], _Opts, _SyntaxLib) ->
    astranaut_traverse_m_v3:return(Node);

map_m_children(F, Node, Subtrees, Opts, SyntaxLib) ->
    SyntaxType = SyntaxLib:type(Node),
    astranaut_traverse_m_v3:bind(
      astranaut_traverse_m_v3:listen_updated(map_m_subtrees(F, Subtrees, Opts#{parent => SyntaxType})),
      fun({_Subtrees1, true}) ->
              astranaut_traverse_m_v3:return(Node);
         ({Subtrees1, false}) ->
              Node1 = SyntaxLib:update_subtrees(Node, Subtrees1),
              astranaut_traverse_m_v3:return(Node1)
      end).
...
m_subtrees(F, {transformer, Subtree, Transformer}, Opts) ->
    astranaut_traverse_m_v3:bind(
      ...
      fun(Subtree1) ->
              ...
              astranaut_traverse_m_v3:return(Transformed)
      end);
    ...
    astranaut_traverse_m_v3:bind(
      ...
      fun(A) ->
              astranaut_traverse_m_v3:bind(
                deep_sequence_m(MAs),
                fun(As) ->
                        astranaut_traverse_m_v3:return([A|As])
                end)
      end);
deep_sequence_m([]) -> 
    astranaut_traverse_m_v3:return([]);
deep_sequence_m(MA) -> 
    MA.
...
transform_f(F, #{traverse := pre}) ->
    fun(Node, #{step := pre  } = Attr) -> F(Node, Attr);
       ...
       (Node, #{step := post }) -> astranaut_traverse_m_v3:return(Node)
    end;
transform_f(F, #{traverse := post}) ->
    fun(Node, #{step := pre  }) -> astranaut_traverse_m_v3:return(Node);
       (Node, #{step := leaf } = Attr) -> F(Node, Attr);
