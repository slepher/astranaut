...
%%%-------------------------------------------------------------------
-module(astranaut_traverse_m_v3).
...
-export([listen_continue/1, listen_updated/1]).
...
astranaut_traverse_m(#{?STRUCT_KEY := ?BASE_M, return := Return, errors := Errors, warnings := Warnings}) ->
    Inner =
        fun(_Formatter, State, Ctx0) ->
                ...
                state_ok(#{return => Return, state => State, ctx => Ctx2})
        end,
    new(Inner);
astranaut_traverse_m(#{?STRUCT_KEY := ?RETURN_OK, return := Return, error := Error1}) ->
    Inner =
        fun(_Formatter, State, #{file := File} = Ctx) ->
                ...
                state_ok(#{return => Return, state => State, ctx => Ctx, error => Error2})
        end,
    new(Inner);
...
eval(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    astranaut_monad:lift_m(fun({A, _State, _Nodes}) -> A end, run(MA, Formatter, State), astranaut_return_m).
...
exec(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    astranaut_monad:lift_m(fun({_A, State1, _Nodes}) -> State1 end, run(MA, Formatter, State), astranaut_return_m).
...
enodes(#{?STRUCT_KEY := ?TRAVERSE_M} = MA, Formatter, State) ->
    ...
    astranaut_monad:lift_m(F, run(MA, Formatter, State), astranaut_return_m).
