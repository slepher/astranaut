...
-export([
    ...
    handle_form/3,
    prepare_target/3,
    validate_generated/2,
    remove_declarations/2,
    finish_attribute_pass/3,
    run_function_pass/3,
    new/0, new/1,
    register/5,
    prepare_declaration/3,
    need_callable/3,
    compile_plan/2,
    cache_expanded/5,
    commit_compiled/3,
    ...
    local_macros/1,
    retained_form_ids/1,
    nonclosure_retain_roots/2,
    expand_final_functions/4,
    source_view/2,
    env_fingerprint/3,
    reject_locked_mutation/2,
    safe_load/3,
    finalize_plan/1,
    module_name/1,
    form_id/1,
...
-type expansion_record() ::
    #{
        ...
        results_by_input :=
            #{
                term() => #{
                    whitelist := disabled | ordsets:ordset(fa()),
                    result := term()
                }
            }
    }.
...
validate_function_with_arity({Function, Arity} = FA) when
...
validate_closure_roots_defined(Options, ClauseMap) ->
    Missing = [
        FA
     || FA <- maps:get(closure_roots, Options, []),
...
finish_attribute_pass(Forms, CompileOpts, State) ->
    ...
    do([
     ...
     || WorkflowContext = #{
        ...
        return({Forms1, RetainedFunctionIds, State1, RetainWarnings})
    ]).
...
remove_final_skip_form(
    ...
    case RemainingFAs of
        [] -> [];
        _ -> [{attribute, Pos, compile, {nowarn_unused_function, RemainingFAs}}]
    end;
...
retain_roots(Forms) ->
    lists:foldl(
        fun
            ({attribute, _Pos, local_macro_retain, Attr}, Acc) ->
                retain_fas(Attr, Acc);
...
retain_warnings(Forms, State) ->
    ...
    lists:flatmap(
        fun
            ({attribute, Pos, local_macro_retain, Attr}) ->
...
retain_root_warnings(Pos, Undefined, Nonclosure) ->
    [
        {Pos, astranaut_macro, {undefined_local_macro_retain, Undefined}}
     || Undefined =/= []
    ] ++
        [
            {Pos, astranaut_macro, {ineffective_local_macro_retain, Nonclosure}}
         || Nonclosure =/= []
        ].
...
retain_fas({Name, Arity} = FA, Acc) when
        ...
        none ->
            ...
            case
                closures(
                    ...
                    Options,
                    FormMap,
                    maps:get(macro_map, MacroEnvironment)
                )
            of
...
local_macro_entries(MacroMap) ->
    maps:filter(
        fun
            (_Key, #{macro_source := local_macro}) -> true;
            (_Key, _Macro) -> false
        end,
        MacroMap
    ).
...
compile_plan(Needed, #{local_macros := Macros} = State) ->
    case needed_entries(Needed, Macros) of
        {error, _} = Error ->
            Error;
        Entries ->
            Ordered = lists:keysort(1, [
                {maps:get(order, Entry), FA, Entry}
             || {FA, Entry} <- Entries
            ]),
            {ok,
    ...
    case whitelist_matches(CanonicalWhitelist, Whitelist) of
        ...
        true ->
            ...
            case maps:find(Fingerprint, Results0) of
                ...
                _ ->
                    Record = #{
                        canonical_whitelist => CanonicalWhitelist,
...
finalize(RetainRoots, #{local_macros := Macros} = State) ->
    ...
    {LocalEnv, Skip, State#{
        retain_roots => ordsets:from_list(RetainRoots), retained_form_ids => Retained
    }}.
...
prepare_final_function_task(
    ...
    case
        cached_final_expansion(
            FormId, Fingerprint, WhitelistControl, State
        )
    of
        {ok, ExpandedForm} ->
            {Tasks, maps:put(FormId, ExpandedForm, CompiledForms), TaskMetadata};
        error ->
            Task0 = #{
                form => OriginalForm,
            ...
            {maps:put(FormId, Task, Tasks), CompiledForms, maps:put(FormId, Metadata, TaskMetadata)}
    end.
            ...
            case maps:find(FormId, TaskMetadata) of
                ...
                {ok, #{
                    ...
                    case maps:find(FormId, TaskResults) of
                        error ->
                            astranaut_return:return(
                                {maps:put(FormId, FallbackForm, CompiledAcc), StateAcc}
                            );
                        {ok, #{
                            form := ExpandedForm,
    ...
    case
        [
         ...
         || Form <- Forms,
            begin
                Id = form_id(Form),
                Id =/= undefined andalso maps:is_key(Id, Frozen)
            end
        ]
    of
        [] -> ok;
...
-spec compile_boundary(compilation_boundary(), workflow_context(), state()) ->
    astranaut_return:struct(state()).
compile_boundary(
    #{members := Members, requests := Requests} = Boundary,
    WorkflowContext,
    State
) ->
    ...
    case
        maps:is_key(
            BoundaryKey, maps:get(committed_boundaries, State, #{})
        )
    of
        true ->
            astranaut_return:return(State);
...
prepare_request(#{forms := FrozenForms} = Request, WorkflowContext, State) ->
    ...
    astranaut_return:foldl_m(
        ...
        State,
        maps:keys(FrozenForms)
    ).

-spec prepare_request_form(
    form_id(),
    [term()],
    expansion_request(),
    workflow_context(),
...
select_local_macro_forms(LocalModule, LocalMacroRelatedFunctions, Forms) ->
    lists:reverse(
        lists:foldl(
            fun
                ({attribute, Pos, module, _Module}, Acc) ->
                    [{attribute, Pos, module, LocalModule} | Acc];
                ({function, _Pos, Name, Arity, _Clauses} = Node, Acc) ->
                    append_if(
                        ordsets:is_element({Name, Arity}, LocalMacroRelatedFunctions), Node, Acc
                    );
...
-spec compile_local_macro_forms(
    ordsets:ordset(fa()),
    [term()],
    [compile:option()]
) ->
...
closures(FAs, Options, FormMap, MacroMap) ->
    ...
    case
        [
         ...
         || FA <- ClosureRoots,
            not maps:is_key(
                {function, element(1, FA), element(2, FA)}, FormMap
            )
        ]
    of
        [] ->
        ...
        Missing ->
            {error, {invalid_closure_roots, Missing}}
    end.
...
analyze_closure_function(
    ...
    case maps:find(FormId, FunctionCallAnalysis) of
        ...
        error ->
            ...
            {Analysis, maps:put(FormId, Analysis, FunctionCallAnalysis)}
    end.
...
freeze_fa({Name, Arity}, FormMap, Acc) ->
    ...
    Acc1 =
        case maps:find(FunctionId, FormMap) of
            {ok, FunctionForm} -> maps:put(FunctionId, FunctionForm, Acc);
            error -> Acc
        end,
    case maps:find(SpecId, FormMap) of
        {ok, SpecForm} -> maps:put(SpecId, SpecForm, Acc1);
        error -> Acc1
    end.
...
closure_ids(Closure, FormMap) ->
    lists:foldl(
        fun(FA, Acc) ->
            Ids = [
                {function, element(1, FA), element(2, FA)}, {spec, element(1, FA), element(2, FA)}
            ],
...
needed_entries(all, Macros) ->
    maps:to_list(Macros);
needed_entries(FA, Macros) ->
    case maps:find(FA, Macros) of
        error ->
            {error, {undefined_local_macro, FA}};
        {ok, #{order := RequestedOrder}} ->
            ...
            [
                {F, Entry}
             || {F, #{order := Order} = Entry} <- maps:to_list(Macros),
                Order =< RequestedOrder
            ]
    end.

-spec compilation_boundaries(
    fa() | all,
    [{non_neg_integer(), fa(), map()}],
    #{form_id() => term()}
) ->
