...
-type astranaut_traverse_m_inner(S, A) :: 
        fun((astranaut_traverse_m_error:formatter(), S) -> astranaut_traverse_m_state(S, A)).
...
%% API
-export([new/0, new/1, new_state/2, new_state/3, run/3]).
...
-export([update_file/1, update_line/1]).
...
new(Inner) when is_function(Inner, 2) ->
...
run(#{'__struct__' := ?MODULE, inner := Inner}, Formatter, State) ->
    Inner(Formatter, State).

bind(MA, KMB) ->
    Inner = 
        fun(Formatter, State) ->
                #{'__struct__' := ?STATE, return := A, state := State1, error := Error1} = 
                    run(MA, Formatter, State),
                #{'__struct__' := ?STATE, return := B, state := State2, error := Error2} =
                    run(KMB(A), Formatter, State1),
                Error3 = astranaut_traverse_m_error:merge(Error1, Error2),
                new_state(B, State2, Error3)
        end,
    new(Inner).

return(A) ->
    Inner = 
        fun(_Formatter, State) ->
                new_state(A, State)
        end,
    new(Inner).
...
update_line(Line) ->
    Inner = 
        fun(Formatter, State) ->
                ErrorState = astranaut_traverse_m_error:new_line(Line, Formatter),
                new_state(ok, State, ErrorState)
        end,
    new(Inner).
