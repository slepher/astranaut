    ...
    try
        ...
        LegacyForms =
            [
                ...
                {attribute, 2, compile, {parse_transform, astranaut_lib_source_transformer}}
            ],
        ?assertEqual(
            {"legacy_source.erl", 2},
            astranaut_lib:analyze_transform_file_pos(
...
lib_transform_file_pos_compat_contracts(_Config) ->
    ...
    MissingForms =
        [
            FileForm,
            {attribute, 5, compile, [debug_info, {parse_transform, other_transformer}]}
        ],
    ?assertEqual(
        {File, 0},
        astranaut_lib:analyze_transform_file_pos(
                ...
                {clause, 3, [], [], [
                    {call, 3, {atom, 3, do}, [
                        lc(
                            ...
                            [
                                {generate, 4, {var, 4, 'A'},
                                    {tuple, 4, [{atom, 4, just}, {integer, 4, 1}]}},
...
do_error_contracts(_Config) ->
    ...
    EmptyError = astranaut_do:do(Empty, #{
        monad => astranaut_monad,
    ...
    LastGenerateError = astranaut_do:do(LastGenerate, #{
        monad => astranaut_monad,
        monad_fail => astranaut_monad
    }),
    ?assertEqual(
        [last_generate_expression],
...
walk_return_contracts(_Config) ->
    ...
    VariantCases =
        [
            ...
            {{warnings, [warn_1, warn_2]}, #{warnings => [warn_1, warn_2]}},
            {{warnings, replacement, [warn_1, warn_2]}, #{
                return => replacement, warnings => [warn_1, warn_2]
            }},
            {{error, error_1}, #{errors => [error_1]}},
            {{errors, [error_1, error_2]}, #{errors => [error_1, error_2]}},
            {{error, replacement, error_1}, #{return => replacement, errors => [error_1]}},
    ...
    ?assertException(
        exit,
        {errors_should_be_list, not_a_list},
        astranaut:walk_return(#{errors => not_a_list})
    ),
    ?assertException(
        exit,
        {warnings_should_be_list, not_a_list},
...
return_error_contracts(_Config) ->
    ...
    ?assertException(
        exit,
        {incompatable_value, {invalid_return_shape}},
...
error_state_contracts(_Config) ->
    ...
    Formatted2 = astranaut_error:with_all_error(
        fun
            (old) -> new;
            (Other) -> Other
        end,
    ...
    File3 = astranaut_error:with_all_warning(
        fun
    ...
    Pending = astranaut_error:append_formatted_errors(
        [{1, ?MODULE, no_file}], astranaut_error:new()
    ),
...
struct_negative_contracts(_Config) ->
    ...
    ?assertEqual(
        #{name => value}, astranaut_struct:to_map(test, #{'__struct__' => test, name => value})
    ).
...
compile_meta_success_contract(_Config) ->
    ...
    ?assert(
        lists:any(
            fun
                ({attribute, _, compile, {parse_transform, compile_meta_identity_transformer}}) ->
                    true;
                (_) ->
                    false
            end,
            compile_meta_success_mod:forms()
        )
    ).

compile_meta_warning_contract(_Config) ->
    Forms = meta_forms(compile_meta_warning_mod, [compile_meta_warning_transformer], [
        silent_warning
    ]),
    OutputForms = astranaut_compile_meta_transformer:parse_transform(Forms, []),
...
compile_meta_invalid_and_undefined_contracts(_Config) ->
    InvalidForms = meta_forms(compile_meta_invalid_mod, [compile_meta_invalid_transformer], [
        silent_warning
    ]),
    ...
    UndefinedForms = meta_forms(compile_meta_undefined_mod, [compile_meta_missing_transformer], [
        silent_warning
    ]),
    UndefinedOutput = astranaut_compile_meta_transformer:parse_transform(UndefinedForms, []),
    {module, compile_meta_undefined_mod} = compile_and_load(
        compile_meta_undefined_mod, UndefinedOutput
    ),
    ...
    [
        {_File2, [
            {none, compile,
                {parse_transform, compile_meta_crash_transformer, {error, injected_crash, _}}}
        ]}
    ] =
...
compile_meta_option_and_compile_contracts(_Config) ->
    ...
    ErrorForms =
        [
            ...
            {function, 5, value, 0, [{clause, 5, [], [], [{var, 5, 'Unbound'}]}]}
        ],
    ErrorOutput =
