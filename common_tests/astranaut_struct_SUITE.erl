...
all() ->
    [
        ...
        test_from_map_missing_name,
        test_update_missing_name,
        test_update_fail,
        test_parse_transform_import_contract,
        test_format_error_contract,
        test_compile_struct_formatter_present,
        test_compile_enforce_fail,
        test_compile_non_record_fail,
        test_compile_unknown_fields_fail,
        test_compile_missing_enforce_key_fail,
...
test_parse_transform_import_contract(_Config) ->
    ...
    ?assertEqual(
        ...
        length(
            [
                ok
             || {attribute, _, import_macro, Import} <- TransformedWithImport,
...
test_format_error_contract(_Config) ->
    ...
    ?assertEqual(
        io_lib:write(unexpected_reason),
        astranaut_lib:format_error(
            unexpected_reason,
            fun astranaut_struct_transformer:format_error/1
        )
    ),
    ?assertEqual(
        Text,
    ...
    ?assertEqual(
        astranaut_lib:format_default_error(Deep),
...
test_compile_unknown_fields_fail(Config) ->
    assert_compile_reasons(
        astranaut_struct_unknown_fields_fail,
        Config,
        [
            {undefined_record_field, test, unknown},
            {undefined_record_field, test, unknown},
...
test_compile_missing_enforce_key_fail(Config) ->
    assert_compile_reasons(
        astranaut_struct_missing_enforce_fail,
        Config,
        [{missing_enforce_keys, test, [name]}]
    ).
...
test_compile_invalid_struct_name_fail(Config) ->
    assert_compile_reasons(
        astranaut_struct_invalid_name_fail,
        Config,
        [{invalid_struct_name, 42}]
    ).
...
attributes_with_line(Attribute, Forms) ->
    lists:reverse(
        lists:foldl(
            fun
                ({attribute, Line, Attr, Value}, Acc) when Attr =:= Attribute ->
                    [{Line, Value} | Acc];
                (_Node, Acc) ->
                    Acc
            end,
